...
def run_simulation(bandit, true_rewards, num_iterations, arm_chart, regret_chart, prob_chart, selection_prob_chart):
    ...
    for i in range(num_iterations):
        ...
        if (i+1) % 100 == 0:  # update every 100 iterations
            ...
            regret_chart.line_chart(cumulative_regrets[:i + 1])
...
def main():
    ...
    **Exploitation** involves choosing the arm that is estimated to have the highest reward probability based on the information collected so far. It maximizes the immediate reward.
    ...
    st.subheader('Estimated Reward Probabilities for each Arm')
    prob_chart = st.empty()
    ...
    rewards, cumulative_regrets = run_simulation(bandit, true_rewards,
                                                 num_iterations, arm_chart,
                                                 regret_chart, prob_chart,
                                                 selection_prob_chart)

    regret_fig = plot_regret(cumulative_regrets, num_iterations)
    st.pyplot(regret_fig)
